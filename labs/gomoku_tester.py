...
# put your filename here (pls for the love of god run this shit in the same folder as your file)
gomoku = __import__("gomoku")

HEADER = 16
DELAY = 0.01  # pls don't ddos me
PORT = 5555
FORMAT = 'utf-8'
...
class client():
    ...
    def continuous_analysis(self):  # thanks Balaji!
        ...
        while yourAnalysis == serverAnalysis:
            ...
            f = io.StringIO()
            # temporarily redirect console output to a string buffer
    ...
    def generate_random_board(self):
        ...
        for i in range(randint(5, 30)):
            ...
            yeee = ('w', 'b')
            try:
                gomoku.put_seq_on_board(board, randint(0, 7), randint(
                    0, 7), randint(-1, 1), randint(0, 1), randint(2, 5), yeee[randint(0, 1)])
            except:
                i -= 1
        return board
        '''
