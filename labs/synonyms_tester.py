# Don't even try looking at this code it's f****** disgusting
...
synonyms = __import__("project3")  # put your filename here

HEADER = 16
DELAY = 0.0  # hehehehe
PORT = 5555
FORMAT = 'utf-8'
HOST_IP = '35.171.50.193'  # hackers. Challenge accepted aight?
        ...
        try:
            ...
            while (True or cnt > 1000):
                ...
                try:
                    ...
                    try:
                        # Ugly fix
                        idx = re.match(r"\d+", msg).span(0)[1]
                        msg_length = int(msg[:idx])
                        temp += msg[idx:]
                        break
                    except Exception as e:
                        print(
                            "Something went wrong. Try running it again without changing anything, it might just fix itself. (Trust me, we do this all the time.)")
                        sys.exit(1)
...
class client():
    ...
    def check(self):
        ...
        f.write(sentences)
        f.close()
        user_dict = synonyms.build_semantic_descriptors_from_files(
            ["sample_case.txt"])
        good = True
        for word in mandarin_dict.keys():
            ...
            for value in values.keys():
                try:
                    ...
                    print(
                        f"An error occured when trying to index [{word}][{value}]")
                    print('\n')
                    good = False
                    break
        if(good):
                    ...
                    for word2 in words:
                        if(word2 in mandarin_cos[word1].keys()):
                            if(round(mandarin_cos[word1][word2], 5) != round(synonyms.cosine_similarity(mandarin_dict[word1], mandarin_dict[word2]), 5)):
                                ...
                                print(
                                    f"Cosine Similarity for {word1} and {word2} don't match!")
                                print('\n')
                                print("MRMANDARIN'S VALUE:",
                                      mandarin_cos[word1][word2])
                                print("YOUR VALUE:", synonyms.cosine_similarity(
                                    mandarin_dict[word1], mandarin_dict[word2]))
                                print('\n')
    ...
    def continuous_run(self):
        ...
        while(good):
            ...
            f.write(get_tests)
            f.close()
            result = synonyms.run_similarity_test("sample_test.txt", synonyms.build_semantic_descriptors_from_files([
                                                  "sample_case.txt"]), synonyms.cosine_similarity)
            if(int(result) != 100):
                print("HMMM SOMETHING SEEMS TO BE OFF")
                print(
                    "Can't really pinpoint why tbh but I'll provide you with the debugging info tho...")
                print('\n')
                print(
                    "Checkout sample_case.txt made in your folder for the sentences used")
                print(
                    "Checkout sample_test.txt for the testing of run_similarity_test (also in your folder now)")
                print('\n')
